# [IGS](https://ingamestudios.com) UE5 Style Guide() {
...
Every section of this style guide is numbered for both easy reference and easy linking. You can link to any section directly by simply append a hash tag and the section number to the end of the link.
...
| ----------------------- | ---------- | ---------- | -------------------------------- |
| Texture                 | T_         |            |                                  |
| Texture (Diffuse/Albedo/Base Color)| T_ | _DIFF     |                                  |
| Texture (Normal)        | T_         | _NORM         |                                  |
...
It is common practice to pack multiple layers of texture data into one texture. An example of this is packing Emissive, Roughness, Ambient Occlusion together as the Red, Green, and Blue channels of a texture respectively. To determine the suffix, simply stack the given suffix letters from above together, e.g. `_ARM`. 

Packing 4 channels of data into a texture (RGBA) is not recommended as a texture with an alpha channel incurs double the overhead than one without. See the comparison on the right, memory usage is doubled by the inclusion of the alpha channel.

Note: When using the default renderer in unreal (deferred) is its generally speaking best to avoid having transparent surfaces share texture space and material with opaque surfaces, as the renderer will have to check each polygon for transparency, thus wasting precious clock cycles on checking surfaces that should have been alrady culled from this process.	
